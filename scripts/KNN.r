...
#write.csv(home_flat, file = "original_data.csv", row.names = FALSE)
...
#Adds column numbers to the observations
...
#########################
# Making variables categorical and refactorization
#########################
...
conf_int <- predict(model_1_no_outliers, home_flat_20,
                    interval = "confidence")
...
pred_int_4.2
#Plot the predictions against the real sales prices against a straight line
plot(pred_int_4.2[,1], log(home_flat_20_all_energy_labels$Pris_Salg), xlab = "Predicted values of sales price (in log DKK)", ylab = "Sales price (in log DKK)", col = "#3399cc", pch = 20,)
par(col = "gray", lty = 1)
grid()
abline(0, 1, lwd = 2, col = "#ff6347")
lines(pred_int_4.2[,1], pred_int_4.2[,2],
      col = "chocolate1", lwd = 2)
lines(pred_int_4.2[,1], pred_int_4.2[,3],
      col = "chocolate1", lwd = 2)
lines(pred_int_4.2[,1], conf_int[,2],
      col = "burlywood4", lwd = 2)
lines(pred_int_4.2[,1], conf_int[,3],
      col = "burlywood4", lwd = 2)
